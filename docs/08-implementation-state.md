...
## EC-121A — Implement application theme system

- [ ] Define light theme design tokens.
- [ ] Define dark theme design tokens.
- [ ] Configure theme provider/state management.
- [ ] Respect the user's system preference on first visit.
- [ ] Persist the user's selected theme preference.
- [ ] Prevent theme flash during initial application load.
- [ ] Ensure shared UI components consume theme tokens rather than hardcoded colors.
- **Done when:** the application can consistently render in light and dark mode, and the selected preference persists across reloads.
...
- [ ] Add accessible light/dark mode toggle.
- [ ] Display the current theme state clearly.
- **Done when:** teacher/student nav differs correctly and the user can switch themes from the application shell.
...
## EC-224 — Theme and visual consistency audit

- [ ] Verify all pages in light mode.
- [ ] Verify all pages in dark mode.
- [ ] Verify text contrast in both themes.
- [ ] Verify borders, surfaces, inputs, dialogs, dropdowns, and overlays in both themes.
- [ ] Verify charts, timeline markers, Echo types, hotspots, and status indicators remain understandable in both themes.
- [ ] Verify no functionality relies solely on color in either theme.
- [ ] Verify theme persistence across navigation and page reload.
- **Done when:** all EchoClass V1 screens and core interactions are visually consistent and usable in both light and dark mode.
