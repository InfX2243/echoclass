...
export function useMyEchoes() { return useQuery({ queryKey: echoKeys.mine(), queryFn: echoApi.listMine }); }
export function useLessonEchoes(lessonId: string) { return useQuery({ queryKey: echoKeys.lesson(lessonId), queryFn: () => echoApi.listForLesson(lessonId), enabled: !!lessonId }); }
