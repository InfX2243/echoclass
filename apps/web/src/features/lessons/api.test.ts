...
describe('loadAuthorizedLesson', () => {
  it('loads lesson metadata, playback access, and timeline context', async () => {
    ...
    expect(client.getLesson).toHaveBeenCalledWith('lesson-1');
    expect(client.getPlaybackAccess).toHaveBeenCalledWith('lesson-1');
    expect(client.getTimelineContext).toHaveBeenCalledWith('lesson-1');
  });
