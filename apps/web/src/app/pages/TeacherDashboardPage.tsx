...
export function TeacherDashboardPage() {
  return <AppShell><div className="space-y-9">
    ...
    <section className="grid gap-4 sm:grid-cols-3">
      {[['Classes', '2', BookOpen], ['Students', '52', Users], ['Recent Echo activity', '25', Flame]].map(([label, value, Icon]) => <article key={String(label)} className="rounded-xl border bg-card p-5"><Icon className="size-5 text-muted-foreground" /><p className="mt-5 text-2xl font-semibold">{value as string}</p><p className="mt-1 text-sm text-muted-foreground">{label as string}</p></article>)}
    </section>
