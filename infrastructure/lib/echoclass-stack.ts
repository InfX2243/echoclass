...
export class EchoClassStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: EchoClassStackProps) {
    ...
    const cognitoUserPoolId = 'ap-southeast-2_pIm3nrBSz';
    const cognitoAppClientId = '7e0e0adbob78cmgb05ots0pl8k';
    ...
    const applicationTable = new dynamodb.Table(this, 'ApplicationTable', {
      ...
      encryption: dynamodb.TableEncryption.AWS_MANAGED, pointInTimeRecoverySpecification: { pointInTimeRecoveryEnabled: true },
    ...
    const mediaBucket = new s3.Bucket(this, 'MediaBucket', { bucketName: `echoclass-${environmentName}-media-${this.account}`, blockPublicAccess: s3.BlockPublicAccess.BLOCK_ALL, encryption: s3.BucketEncryption.S3_MANAGED, enforceSSL: true, objectOwnership: s3.ObjectOwnership.BUCKET_OWNER_ENFORCED, removalPolicy: isProduction ? cdk.RemovalPolicy.RETAIN : cdk.RemovalPolicy.DESTROY, autoDeleteObjects: !isProduction, lifecycleRules: [{ id: 'AbandonedUploads', enabled: true, prefix: 'uploads/pending/', expiration: cdk.Duration.days(7) }] });
    const mediaOriginAccessControl = new cloudfront.CfnOriginAccessControl(this, 'MediaOriginAccessControl', { originAccessControlConfig: { name: `EchoClass-${environmentName}-MediaOAC`, description: 'CloudFront access to private EchoClass media', originAccessControlOriginType: 's3', signingBehavior: 'always', signingProtocol: 'sigv4' } });
    const mediaDistribution = new cloudfront.CfnDistribution(this, 'MediaDistribution', { distributionConfig: { enabled: true, comment: `EchoClass ${environmentName} private media delivery`, defaultRootObject: '', priceClass: 'PriceClass_100', origins: [{ id: 'MediaBucketOrigin', domainName: mediaBucket.bucketRegionalDomainName, originAccessControlId: mediaOriginAccessControl.ref, s3OriginConfig: { originAccessIdentity: '' } }], defaultCacheBehavior: { targetOriginId: 'MediaBucketOrigin', viewerProtocolPolicy: 'redirect-to-https', allowedMethods: ['GET', 'HEAD'], cachedMethods: ['GET', 'HEAD'], compress: true, forwardedValues: { queryString: true, cookies: { forward: 'none' } } }, restrictions: { geoRestriction: { restrictionType: 'none' } }, viewerCertificate: { cloudFrontDefaultCertificate: true } } });
    ...
    const apiLogGroup = new logs.LogGroup(this, 'ApiLogGroup', { logGroupName: `/aws/lambda/EchoClass-${environmentName}-api`, retention: logs.RetentionDays.ONE_WEEK, removalPolicy: isProduction ? cdk.RemovalPolicy.RETAIN : cdk.RemovalPolicy.DESTROY });
    const apiHandler = new lambda.Function(this, 'ApiHandler', {
      functionName: `EchoClass-${environmentName}-api`, runtime: lambda.Runtime.NODEJS_22_X, handler: 'api-handler.handler', code: lambda.Code.fromAsset(lambdaAssetPath), timeout: cdk.Duration.seconds(10), memorySize: 256, tracing: lambda.Tracing.PASS_THROUGH,
      environment: { TABLE_NAME: applicationTable.tableName, MEDIA_BUCKET_NAME: mediaBucket.bucketName, MEDIA_DISTRIBUTION_DOMAIN: mediaDistribution.attrDomainName, COGNITO_REGION: cognitoRegion, COGNITO_USER_POOL_ID: cognitoUserPoolId, COGNITO_APP_CLIENT_ID: cognitoAppClientId }, logGroup: apiLogGroup,
    });
    ...
    httpApi.addRoutes({ path: '/{proxy+}', methods: [apigatewayv2.HttpMethod.ANY], integration: new HttpLambdaIntegration('ProtectedIntegration', apiHandler) });
