...
const normalizePath = (path) => path.replace(/^\/api\/v1(?=\/|$)/, '') || '/';
const getHeader = (headers, name) => { if (!headers || typeof headers !== 'object') return undefined; const wanted = name.toLowerCase(); const key = Object.keys(headers).find((candidate) => candidate.toLowerCase() === wanted); return key ? headers[key] : undefined; };
...
export const handler = async (event) => {
  ...
  try {
    ...
    log('bearer token verified', { subject: principal.subject, username: principal.username, role: principal.claims?.['custom:role'] });
    const user = await getOrCreateUser(principal);
    log('application user resolved', { userId: user.userId, role: user.role });
    if (method === 'GET' && path === '/me') { log('returning /me response', { userId: user.userId }); return json(200, { user: { id: user.userId, username: user.username, email: user.email ?? null, role: user.role ?? null, createdAt: user.createdAt } }); }
    ...
    if (classLessonsId) { if (method !== 'GET' && method !== 'POST') throw error(404, 'NOT_FOUND', 'Route not found'); if (method === 'POST') { if (user.role !== 'TEACHER' || !(await teacherOwnsClass({ classId: classLessonsId, teacherId: user.userId }))) throw error(404, 'NOT_FOUND', 'Class not found'); const body = parseBody(event); const title = requireText(body.title, 'title', { max: 160 }); const description = body.description === undefined ? undefined : requireText(body.description, 'description', { min: 0, max: 2000 }); return json(201, { lesson: await createLesson({ classId: classLessonsId, teacherId: user.userId, title, description }) }); } const isTeacher = user.role === 'TEACHER' && await teacherOwnsClass({ classId: classLessonsId, teacherId: user.userId }); const isStudent = user.role === 'STUDENT' && !!(await getClassForStudent(classLessonsId, user.userId)); if (!isTeacher && !isStudent) throw error(404, 'NOT_FOUND', 'Class not found'); return json(200, { lessons: await listLessonsForClass({ classId: classLessonsId, publishedOnly: isStudent }) }); }
    ...
    if (lessonId && method === 'PATCH') { if (user.role !== 'TEACHER') throw error(403, 'FORBIDDEN', 'Only teachers can update lessons'); const body = parseBody(event); if (body.title === undefined && body.description === undefined) throw error(400, 'VALIDATION_ERROR', 'At least one field must be provided'); const title = body.title === undefined ? undefined : requireText(body.title, 'title', { max: 120 }); const description = body.description === undefined ? undefined : requireText(body.description, 'description', { min: 0, max: 2000 }); const lesson = await updateLesson({ lessonId, teacherId: user.userId, title, description }); if (!lesson) throw error(404, 'NOT_FOUND', 'Lesson not found'); return json(200, { lesson }); }
  ...
  } catch (caught) { log('request failed', { statusCode: caught?.statusCode ?? 500, code: caught?.code ?? 'INTERNAL_ERROR', message: caught?.message }); if (caught?.statusCode && caught.statusCode < 500) return json(caught.statusCode, { error: { code: caught.code ?? 'BAD_REQUEST', message: caught.message } }); console.error('API request failure', caught); return json(500, { error: { code: 'INTERNAL_ERROR', message: 'Unable to process request' } }); }
};
