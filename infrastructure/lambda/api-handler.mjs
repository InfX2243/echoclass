...
export const handler = async (event) => {
  ...
  try {
    ...
    if (classLessonsId) {
      ...
      if (method === 'POST') { if (user.role !== 'TEACHER' || !(await teacherOwnsClass({ classId: classLessonsId, teacherId: user.userId }))) throw error(404, 'NOT_FOUND', 'Class not found'); const body = parseBody(event); const title = requireText(body.title, 'title', { max: 160 }); const description = body.description === undefined ? undefined : requireText(body.description, 'description', { min: 0, max: 2000 }); return json(201, { lesson: await createLesson({ classId: classLessonsId, teacherId: user.userId, title, description }) }); }
      const isTeacher = user.role === 'TEACHER' && await teacherOwnsClass({ classId: classLessonsId, teacherId: user.userId }); const isStudent = user.role === 'STUDENT' && !!(await getClassForStudent(classLessonsId, user.userId)); if (!isTeacher && !isStudent) throw error(404, 'NOT_FOUND', 'Class not found'); return json(200, { lessons: await listLessonsForClass({ classId: classLessonsId, publishedOnly: isStudent }) });
    }
