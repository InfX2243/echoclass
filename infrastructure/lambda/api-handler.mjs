...
export const handler = async (event) => {
  ...
  log('request received', { method, rawPath, normalizedPath: path, hasBody: Boolean(event.body), bodyLength: typeof event.body === 'string' ? event.body.length : undefined, hasAuthorizationHeader: Boolean(event.headers?.authorization ?? event.headers?.Authorization) });
